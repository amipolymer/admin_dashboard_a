...
class EmployeeNewJoinerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            "emp_cv_file" => "required|file|mimes:pdf,doc,docx",
            "emp_interview_evaluation_file" => "required|file|mimes:pdf,doc,docx",
            "emp_mrf_file" => "required|file|mimes:pdf,doc,docx",
        ]);
    ...
    public function viewOffer($id)
    {
        ...
        // dd($id);
        $employee = EmployeesNewJoiner::findOrFail($id);
        return view('pdf.offer_letter', compact('employee'));
    }
    public function previewOffer($id)
    {
        // $id = decrypt($id);
        $employee = EmployeesNewJoiner::findOrFail($id);
        return view('pdf.offer_letter_pdf', compact('employee'));
    }
    ...
    // {
    //     dd($request->all());
    //     logger()->info('storeSignature called with data: ', $request->all());
    //     $request->validate([
    //         'emp_id' => 'required',
    ...
    //     logger()->info('Employee found: ', ['employee_id' => $employee->id, 'employee_name' => $employee->emp_name]);
    //     $signatureData = null;

    //     //  DRAW SIGNATURE
    //     if ($request->signature) {
    //         $signatureData = $request->signature;
    //         logger()->info('Signature data received from drawing: ', ['signature_length' => strlen($signatureData)]);
    //     }

    //     //  UPLOAD SIGNATURE (AUTO HANDLE IMAGE TYPE)
    //     if ($request->hasFile('signature_file')) {
    //       logger()->info('Signature file uploaded: ', ['file_name' => $request->file('signature_file')->getClientOriginalName()]);
    //         $file = $request->file('signature_file');
    //         $path = $file->getPathname();

    //         // Auto create image from string (works for jpg/png)
    ...
    //         $signatureData = 'data:image/png;base64,' . base64_encode($pngData);
    //         logger()->info('Signature data processed from uploaded file: ', ['signature_length' => strlen($signatureData)]);
    //     }
    ...
    //     $employee->save();
    //     logger()->info('Signature saved for employee: ', ['employee_id' => $employee->id, 'employee_name' => $employee->emp_name]);
      ...
      public function storeSignature(Request $request)
{
    ...
    // ✅ PRIORITY 1: DRAW SIGNATURE (already PNG)
    if (!empty($request->signature)) {

        $signatureData = $request->signature;

        logger()->info('Drawn signature used');
    }

    // ✅ PRIORITY 2: FILE UPLOAD (simple base64)
    elseif ($request->hasFile('signature_file')) {
        ...
        $mime = $file->getMimeType(); // image/png, image/jpeg

        $signatureData = 'data:' . $mime . ';base64,' . base64_encode($fileData);

        logger()->info('Uploaded signature used');
    }

    // ❌ If nothing provided
    if (!$signatureData) {
        return back()->with('error', 'Please draw or upload signature');
    }
